...
We present {\it a system for automatic conversion among the scene file formats used by Monte Carlo physically-based rendering systems}. 
...
%
Figure~\ref{fig:teaser} illustrates the use of our system to perform automatic conversion of a scene originally modeled for Mitsuba. The image shown on the left is the original Mitsuba rendering. The images on the center and on the right show, respectively, the renderings produced by PBRT v3 and by LuxRender, from the converted scenes files. Note the correct representation of the scene elements, despite the many differences in the scene representations (\eg, different coordinate systems, (\red{what else?}).
