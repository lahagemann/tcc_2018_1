...
%
A {\it material} (\eg, glass, metal, plastic, etc.) may have one or more associated textures. {\it Global emitters} represent \red{area and point} light sources, as well more specific ones such as {\it sun} and {\it sky}.
...
single value of $\eta$ and \textit{k} for all channels, \red{or specify an RGB 
color}. Therefore, correctly converting metal colors between LuxRender and PBRT or Mitsuba is very hard, 
...
LuxRender, a triangle mesh directive must be used, \red{specifying each 
...
PBRT and LuxRender access environment maps using spherical coordinates ($\theta$ and $\phi$), while 
Mitsuba uses Cartesian coordinates (\textit{x}, \textit{y} and \textit{z}).
%, as  shown in Figure \ref{fig:mitdocemitter}. 
Conversion between the two representations can be done by applying a \red{transformation matrix on the 
